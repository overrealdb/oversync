//! SurrealDB source example: query SurrealDB → stdout sink.
//!
//! Run: `cargo run --example surrealdb_source`
//!
//! Uses in-memory SurrealDB — no external dependencies needed.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	// Seed some data in the same in-memory DB (source ns/db)
	db.use_ns("app").use_db("main").await?;
	db.query(
		"CREATE user:alice SET name = 'Alice', email = 'alice@example.com', active = true;
		 CREATE user:bob SET name = 'Bob', email = 'bob@example.com', active = false;
		 CREATE user:carol SET name = 'Carol', email = 'carol@example.com', active = true;",
	)
	.await?;
	db.use_ns("demo").use_db("sync").await?;
	...
	println!("--- SurrealDB source: sync users ---");
	let result = sync.run_once("surreal-app", "users").await?;
	println!(
