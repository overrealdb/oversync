...
export function DurationChart({ cycles }: DurationChartProps) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
      <h3 className="text-sm font-medium text-gray-300 mb-4">Avg Cycle Duration per Source</h3>
      {data.length === 0 ? (
        <p className="text-sm text-gray-500 py-8 text-center">No cycle data yet</p>
      ) : (
        <BarChart
          className="h-56"
          data={data}
